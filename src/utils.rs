...
fn format_lcu_data(data_str:String) -> HashMap<String, String> {
  ...
  for item in col {
      ...
      data_map.insert(temp[0].to_string(), temp[1].replace("\"", "").trim().to_string());
  }
  data_map
}
...
#[test]
fn it_works() {
    let result = format_lcu_data("\"--remoting-auth-token=_BkC3zoDF6600gmlQdUs6w\" \"--app-port=58929\"".to_string());
    let mut assert_res = HashMap::new();
    assert_res.insert("remoting-auth-token".to_string(), "_BkC3zoDF6600gmlQdUs6w".to_string());
